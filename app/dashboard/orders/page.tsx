...
export default function Order() {
  ...
  const handleOrderSubmit = async () => {
    if (!session) {
      // Check if session exists instead of user
    ...
    try {
      const response = await fetch("/api/customerOrder", {
        ...
        body: JSON.stringify({ orderData, userId: session?.user?.id }), // Use optional chaining to avoid undefined error
      });
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {orderData.selectedProducts.map((product: Zuivel) => (
          <div
            key={product.id}
            className="flex w-full items-center justify-between border border-gray-100 rounded-2xl bg-gray-50 p-3 shadow-3xl shadow-xs mb-4"
          >
